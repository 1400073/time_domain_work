...
data = np.load("bias_values.npz")
pos_amp = data["pos_amp"]
pos_phase = data["pos_phase"]
neg_amp = data["neg_amp"]
neg_phase = data["neg_phase"]
...
for i,w in enumerate(wavelengths):
    print("\n\npos_bias:\n")
    pos_amp_current = pos_amp[i*5:i*5+5]
    pos_phase_current = pos_phase[i*5:i*5+5]
    neg_amp_current = neg_amp[i*5:i*5+5]
    neg_phase_current = neg_phase[i*5:i*5+5]
    ...
    for j, weight in enumerate(weights_pos[i*5:i*5+5]):
        ...
        pos_amp_value = pos_amp_current[j]
        pos_phase_value = pos_phase_current[j]
        x_pos_value = x_pos_current[:,j]

        phase_1 = phase_mod(np.abs(weight/15)*pos_amp_value)
        phase_2 = -phase_1/2+np.angle(weight) + pos_phase_value
        ...
        output_pos = results.outputs["o1"]
        output_pos_list.append(output_pos)
        ref = x_pos_value[3950]       
        desired_pos_amp = np.abs(ref * weight)
        desired_pos_phase = np.angle(ref*weight)
        

        print("amplitude: ", desired_pos_amp/np.abs(output_pos[4000]*15))
        print("phase: ", (desired_pos_phase - np.angle(output_pos[4000])))

    print("\n\nneg_bias:\n")
    for j,weight in enumerate(weights_neg[i*5:i*5+5]):
        def phase_mod(amp):
            return 2*np.arccos(amp)
        neg_amp_value = neg_amp_current[j]
        neg_phase_value = neg_phase_current[j]
        x_neg_value = x_neg_current[:,j]

        phase_1 = phase_mod(np.abs(weight/15)*neg_amp_value)
        phase_2 = -phase_1/2+np.angle(weight) + neg_phase_value
        phase_mod1 = Modulator(mod_signal=phase_1*jnp.ones_like(t))
        phase_mod0 = Modulator(mod_signal = 0*jnp.ones_like(t))
        phase_mod2 = Modulator(mod_signal=phase_2*jnp.ones_like(t))
        models = {
        ...
        with open(os.devnull, 'w') as fnull, \
            ...
            contextlib.redirect_stderr(fnull):
                results = time_sim.run(t,inputs, carrier_freq=c/(w*1e-6), dt=dt)
        output_neg = results.outputs["o1"]
        output_neg_list.append(output_neg)
        ref = x_neg_value[3950] 
        desired_neg_amp = np.abs(ref * weight)
        desired_neg_phase = np.angle(ref*weight)
        
        print("amplitude: ", desired_neg_amp/np.abs(output_neg[4000]*15))
        print("phase: ", (desired_neg_phase - np.angle(output_neg[4000])))

np.savez_compressed(
    "output_MZI.npz",
    output_pos_save_real = np.real(np.array(output_pos_list,dtype=np.complex64)),
    output_pos_save_imag = np.imag(np.array(output_pos_list,dtype=np.complex64)),
    output_neg_save_real = np.real(np.array(output_neg_list,dtype=np.complex64)),
    output_neg_save_imag = np.imag(np.array(output_neg_list,dtype=np.complex64)),
)
