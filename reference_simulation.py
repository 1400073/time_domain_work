    ...
    **User-Facing**: Typical usage involves:
    ...
    def build_model(
        ...
        **User-Facing**: This is typically the second step (after __init__).
        ...
        if self.time_system_components is not None and len(self.time_system_components) > 0:
            ...
            if self.passive_subnetlists:
                ...
                for i, circuit in sub_circuit_list.items():
                    ...
                    sorted_ports = sorted(port_map_list[i].keys(), key=lambda p: int(p.lstrip('o')))
            ...
            self.td_netlist["models"]["0"] = self.time_system
            for key,value in self.netlist["ports"].items():
                self.td_netlist["ports"][key] = "0," + key
